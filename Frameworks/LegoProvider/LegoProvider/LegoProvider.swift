...
public class LegoProvider<Target: SugarTargetType>: RxMoyaProvider<Target> {
    ...
    private static func endpointClosure(_ target: Target) -> Endpoint<Target> {
...
extension LegoProvider {

    public func requestWithProgress(_ token: Target, progressHandler: @escaping (Double)-> Void) -> Observable<Response> {
        let response: Observable<Response> = Observable.create { observer in
            let cancellableToken = self.request(token, queue: nil, progress: { (progressResponse: ProgressResponse) in
